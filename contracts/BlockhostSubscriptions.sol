...
contract BlockhostSubscriptions is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Get subscriptions expiring within a time window
     * @dev Useful for sending expiration warnings
     * @param withinSeconds Seconds from now to check
     * @param offset Pagination offset
     * @param limit Maximum results to return
     * @return subscriptionIds Array of subscription IDs expiring within the window
     */
    function getSubscriptionsExpiringSoon(
        uint256 withinSeconds,
        uint256 offset,
        uint256 limit
    ) external view returns (uint256[] memory subscriptionIds) {
        uint256 deadline = block.timestamp + withinSeconds;

        // Count matching subscriptions first
        uint256 count = 0;
        for (uint256 i = 1; i < nextSubscriptionId; i++) {
            Subscription memory sub = _subscriptions[i];
            if (!sub.cancelled &&
                sub.expiresAt > block.timestamp &&
                sub.expiresAt <= deadline) {
                count++;
            }
        }
        ...
        subscriptionIds = new uint256[](resultSize);
        uint256 found = 0;
        uint256 skipped = 0;

        for (uint256 i = 1; i < nextSubscriptionId && found < resultSize; i++) {
            Subscription memory sub = _subscriptions[i];
            if (!sub.cancelled &&
                sub.expiresAt > block.timestamp &&
                sub.expiresAt <= deadline) {
                if (skipped >= offset) {
                    subscriptionIds[found] = i;
                    found++;
                } else {
                    skipped++;
                }
    ...
    /**
     * @notice Get expired but not cancelled subscriptions
     * @dev Useful for cleanup/suspension scripts
     * @param offset Pagination offset
     * @param limit Maximum results to return
     * @return subscriptionIds Array of expired subscription IDs
     */
